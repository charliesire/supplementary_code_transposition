...
def MCMC_treat(index_calib, idx_loo, u, mm_list, index_lambda_p, bMINlambda, bMAXlambda, pre_path):
    if idx_loo is None: 
        ...
        Ysimu, Ystd = pd.DataFrame(np.zeros((0,3))), pd.DataFrame(np.zeros((0,3)))
    for i in range(len(lambda_post)):
        ...
        res, res_std = myCODE(lambda_tot, index = [1,2,3],  std_bool = True, vectorize = True, idx_loo = idx_loo, new_x = (idx_loo is not None), mm_list = mm_list) #compute GP means and stds
        if idx_loo is None: 
            Ysimu += res
            Ystd += res_std
        else:
...
def results_embed(index_calib, num_chain, tune_size, size, u, mm_list, results_measures, sigma, index_lambda_p, bMINlambda, bMAXlambda, pre_path, true_values, rngseed, loo = True):
    if not loo: 
        results = MCMC_treat(index_calib = index_calib, idx_loo = None, u = u, mm_list = mm_list,index_lambda_p = index_lambda_p, bMINlambda = bMINlambda, bMAXlambda = bMAXlambda, pre_path = pre_path)
        YYtot = [pd.concat([results[0][ii].iloc[:,idx_y] for ii in range(len(results[0]))], axis=1).transpose() for idx_y in range(3)]
        YYstdtot = [pd.concat([results[1][ii].iloc[:,idx_y] for ii in range(len(results[1]))], axis=1).transpose() for idx_y in range(3)]
    else: 
        list_idx_loo = range(len(results_measures))
        results = Parallel(n_jobs=-1)(delayed(lambda idx_loo: MCMC_treat(index_calib = index_calib, idx_loo = idx_loo, u = u, mm_list = mm_list,index_lambda_p = index_lambda_p, bMINlambda = bMINlambda, bMAXlambda = bMAXlambda, pre_path = pre_path))(idx_loo) for idx_loo in list_idx_loo)
        YYtot = [pd.concat([results[jj][0].iloc[:,idx_y] for jj in list_idx_loo], axis=1) for idx_y in range(3)]
        YYstdtot = [pd.concat([results[jj][1].iloc[:,idx_y] for jj in list_idx_loo], axis=1) for idx_y in range(3)]
